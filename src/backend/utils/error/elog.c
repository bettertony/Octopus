...
set_errcontext_domain(const char *domain)
{
	...
	/* the default text domain is the backend's */
	edata->context_domain = domain ? domain : PG_TEXTDOMAIN("octopus");
...
sanitize_fmt_string(const char *fmt)
{
	...
	sanitized = DirectFunctionCall3(replace_text,
			sanitized, CStringGetTextDatum("postgresql"), CStringGetTextDatum("octopus"));

	sanitized = DirectFunctionCall3(replace_text,
			sanitized, CStringGetTextDatum("PostgreSQL"), CStringGetTextDatum("Octopus"));

	sanitized = DirectFunctionCall3(replace_text,
			sanitized, CStringGetTextDatum("postgres"), CStringGetTextDatum("octopus"));

	sanitized = DirectFunctionCall3(replace_text,
			sanitized, CStringGetTextDatum("Postgres"), CStringGetTextDatum("Pipeline"));

	sanitized = DirectFunctionCall3(replace_text,
			sanitized, CStringGetTextDatum("initdb"), CStringGetTextDatum("octopus-init"));
