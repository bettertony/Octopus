 ...
 *
 * octopusfuncs.c
 *		Functions for Octopus functions
 *
 * Copyright (c) 2013-2015, Octopus
 *
 * IDENTIFICATION
 *	  src/backend/utils/adt/octopusfuncs.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
 ...
 * octopus_stat_get
 *
 * Global Octopus stats. These don't go away when CVs are dropped.
 */
Datum
octopus_stat_get(PG_FUNCTION_ARGS)
{
	FuncCallContext *funcctx;
	HTAB *stats;
 ...
 */
Datum
octopus_views(PG_FUNCTION_ARGS)
{
	FuncCallContext *funcctx;
	HeapTuple tup;
	MemoryContext old;
 ...
 */
Datum
octopus_streams(PG_FUNCTION_ARGS)
{
	FuncCallContext *funcctx;
	HeapTuple tup;
	MemoryContext old;
	...
	while ((tup = heap_getnext(data->scan, ForwardScanDirection)) != NULL)
	{
		...
		tmp = PipelineSysCacheGetAttr(PIPELINESTREAMRELID, tup, Anum_octopus_stream_queries, &isnull);
 ...
 * octopus_version
 *
 * Returns the Octopus version string
 */
Datum
octopus_version(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
octopus_transforms(PG_FUNCTION_ARGS)
{
	FuncCallContext *funcctx;
	HeapTuple tup;
	MemoryContext old;
 ...
 */
Datum
octopus_flush(PG_FUNCTION_ARGS)
{
	int i;
...
#define UPDATE_TTL_TEMPLATE "UPDATE octopus_query SET ttl = %d, ttl_attno = %d WHERE id = %d;"
