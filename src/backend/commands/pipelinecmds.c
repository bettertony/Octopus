 ...
 *
 * octopuscmds.c
 *	  commands specific to Octopus
 *
 * Copyright (c) 2013-2015, Octopus
 *
 * IDENTIFICATION
 *	  src/backend/commands/octopuscmds.c
 *
 *-------------------------------------------------------------------------
 */
...
#include "commands/octopuscmds.h"
		...
		Oid indexoid, Oid pkey_idxoid, SelectStmt *stmt, Query *query)
{
	...
	/*
	 * Record a dependency between the view its octopus_query entry so that when
	 * the view is dropped the octopus_query metadata cleanup hook is invoked.
	 */
	...
	/*
	 * Record a dependency between any strongly typed streams and a octopus_query object,
...
ExecCreateContViewStmt(CreateContViewStmt *stmt, const char *querystring)
{
	...
	/*
	 * Now save the underlying query in the `octopus_query` catalog relation. We don't have relid for
	 * the continuous view yet, since we need this entry for the DefineView call below to succeed.
	 * We'll update it afterwards.
	 *
	 * pqoid is the oid of the row in octopus_query,
...
record_ct_dependencies(Oid pqoid, Oid relid, Oid osrelid, Oid fnoid, SelectStmt *stmt, Query *query, List *args)
{
	...
	/*
	 * Record a dependency between the type its octopus_query entry so that when
	 * the view is dropped the octopus_query meta data cleanup hook is invoked.
	 */
	referenced.classId = RelationRelationId;
	referenced.objectId = relid;
	referenced.objectSubId = 0;
		...
		foreach(lc, args)
		{
			...
			if (!RangeVarIsForStream(rv))
				ereport(ERROR,
						...
						errhint("Arguments to octopus_stream_insert must be streams.")));
