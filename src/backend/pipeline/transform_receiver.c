 ...
 *
 * Copyright (c) 2013-2016, Octopus
 *
 * IDENTIFICATION
 *	  src/backend/octopus/transform_receiver.c
 *
 */
#include "postgres.h"
#include "pgstat.h"
...
#include "octopus/transform_receiver.h"
...
static void
octopus_stream_insert_batch(TransformReceiver *t, TupleTableSlot *slot)
{
	int i;
...
flush_to_transform(struct BatchReceiver *receiver, TupleTableSlot *slot)
{
	...
	if (t->cont_query->tgfn == PIPELINE_STREAM_INSERT_OID || t->os_has_readers)
		octopus_stream_insert_batch(t, slot);
