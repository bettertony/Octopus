 ...
 *
 * IDENTIFICATION
 *    src/backend/octopus/scheduler.c
 *
 *-------------------------------------------------------------------------
 */
#include <math.h>
...
#include "pgstat.h"
#include "octopus/executor.h"
#include "octopus/scheduler.h"
#include "octopus/ipc/pzmq.h"
...
StartContQueryScheduler(void)
{
	...
	case -1:
		ereport(LOG,
				(errmsg("could not fork octopus scheduler process: %m")));
		return 0;
...
#define BACKTRACE_SEGFAULTS
	...
	switch (proc->type)
	{
		...
		case Reaper:
			...
			break;
		default:
			elog(ERROR, "unknown octopus process type: %d", proc->type);
	}

	elog(LOG, "octopus process \"%s\" running with pid %d", GetContQueryProcName(proc), MyProcPid);
	...
	{
		elog(LOG, "octopus process \"%s\" was killed", GetContQueryProcName(proc));
		proc_exit(1);
	}

	elog(LOG, "octopus process \"%s\" shutting down", GetContQueryProcName(proc));
}
...
wait_for_db_workers(ContQueryDatabaseMetadata *db_meta, BgwHandleStatus state)
{
	...
	for (i = 0; i < NUM_BG_WORKERS_PER_DB; i++)
	{
		...
		if (!wait_for_bg_worker_state(cqproc->bgw_handle, state, BG_PROC_STATUS_TIMEOUT))
			elog(WARNING, "timed out waiting for octopus process \"%s\" to reach state %d",
...
terminate_database_workers(ContQueryDatabaseMetadata *db_meta)
{
	...
	elog(LOG, "terminating octopus processes for database: \"%s\"", NameStr(db_meta->db_name));
...
start_database_workers(ContQueryDatabaseMetadata *db_meta)
{
	...
	elog(LOG, "starting octopus processes for database: \"%s\"", NameStr(db_meta->db_name));
	...
	if (!success)
	{
		...
		elog(ERROR, "failed to start some octopus processes");
	}
...
ContQuerySchedulerMain(int argc, char *argv[])
{
	...
	init_ps_display("octopus scheduler process", "", "", "");

	ereport(LOG, (errmsg("octopus scheduler started")));
...
#define BACKTRACE_SEGFAULTS
	...
	ereport(LOG, (errmsg("octopus scheduler shutting down")));
...
signal_cont_query_scheduler(int signal)
{
	...
	for (ntries = 0;; ntries++)
	{
		if (ContQuerySchedulerShmem->pid == 0)
		{
			...
			{
				ereport(NOTICE, (errmsg("could not signal octopus scheduler because it is not running")));
				break;
			}
		}
		else if (kill(ContQuerySchedulerShmem->pid, signal) != 0)
		{
			if (ntries >= 20) /* max wait 2.0 sec */
			{
				ereport(NOTICE, (errmsg("could not signal octopus scheduler for: %m")));
				break;
			}
		}
		else
