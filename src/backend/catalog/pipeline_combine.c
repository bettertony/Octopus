 ...
 *
 * octopus_combine.c
 *	  routines to support manipulation of the octopus_combine relation
 *
 * Copyright (c) 2013-2015, Octopus
 *
 * IDENTIFICATION
 *	  src/backend/catalog/octopus_combine.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
#include "catalog/octopus_combine_fn.h"
 ...
 * GetCombineInfo
 *
 * Get all the octopus_combine information associated with the given aggregate function
 */
void
GetCombineInfo(Oid aggfnoid, Oid *combinefn, Oid *transoutfn, Oid *combineinfn, Oid *statetype)
{
	...
	*statetype = InvalidOid;
	...
	combform = (Form_octopus_combine) GETSTRUCT(combtup);
...
DefineCombiner(Oid aggoid, List *name, List *args, bool oldstyle, List *parameters)
{
	...
	Relation octopus_combine;
	bool nulls[Natts_octopus_combine];
	Datum values[Natts_octopus_combine];
	...
	if (HeapTupleIsValid(combinetup))
	{
		elog(ERROR, "combine function %u already exists for aggregate with final function %u and transition function %u",
				((Form_octopus_combine) combinetup)->combinefn, agg->aggfinalfn, agg->aggtransfn);
	}

	/* we now have the information we need to create an entry in octopus_combine */
	octopus_combine = heap_open(PipelineCombineRelationOid, RowExclusiveLock);

	MemSet(nulls, false, sizeof(nulls));

	values[Anum_octopus_combine_aggfinalfn - 1] = ObjectIdGetDatum(finalfn);
	values[Anum_octopus_combine_transfn - 1] = ObjectIdGetDatum(transfn);
	values[Anum_octopus_combine_transoutfn - 1] = ObjectIdGetDatum(transoutfn);
	values[Anum_octopus_combine_combineinfn - 1] = ObjectIdGetDatum(combineinfn);
	values[Anum_octopus_combine_combinefn - 1] = ObjectIdGetDatum(combinefn);
	values[Anum_octopus_combine_transouttype - 1] = ObjectIdGetDatum(transouttype);

	combinetup = heap_form_tuple(octopus_combine->rd_att, values, nulls);
	result = simple_heap_insert(octopus_combine, combinetup);
	CatalogUpdateIndexes(octopus_combine, combinetup);

	combinetup_oid = HeapTupleGetOid(combinetup);

	heap_freetuple(combinetup);
	heap_close(octopus_combine, NoLock);
	...
	/*
	 * Record a dependency between octopus_combine entry and the aggregate
	 */
...
RemovePipelineCombineById(Oid oid)
{
	...
	if (!HeapTupleIsValid(tuple))
		elog(ERROR, "cache lookup failed for octopus_combine tuple with OID %u", oid);

	simple_heap_delete(octopus_combine, &tuple->t_self);

	ReleaseSysCache(tuple);

	CommandCounterIncrement();

	heap_close(octopus_combine, RowExclusiveLock);
}
