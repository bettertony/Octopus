 ...
 *
 * octopus_stream.c
 *	  routines to support manipulation of the octopus_stream relation
 *
 * Copyright (c) 2013-2015, Octopus
 *
 * IDENTIFICATION
 *	  src/backend/catalog/octopus_stream.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
 ...
 * streams_to_meta
 *
 * Scans the octopus_query catalog table and reads the FROM clause of
 ...
 */
static HTAB *
streams_to_meta(Relation octopus_query)
{
	...
	scandesc = heap_beginscan_catalog(octopus_query, 0, NULL);
...
/*
 * update_octopus_stream_targets_and_desc
 *
 * Serializes the given stream-to-queries mapping into the
 * octopus_stream catalog table along with the given stream TupleDesc.
 */
static List *
update_octopus_stream_catalog(Relation octopus_stream, HTAB *hash)
{
	StreamTargetsEntry *entry;
	...
	while ((entry = (StreamTargetsEntry *) hash_seq_search(&scan)) != NULL)
	{
		...
		bytea *targetsbytes;
		HeapTuple tup;
		Datum values[Natts_octopus_stream];
		bool nulls[Natts_octopus_stream];
		bool replaces[Natts_octopus_stream];
		HeapTuple newtup;

		MemSet(nulls, false, Natts_octopus_stream);
		...
		if (queries)
		{
			...
			values[Anum_octopus_stream_queries - 1] = PointerGetDatum(targetsbytes);
		}
		else
			nulls[Anum_octopus_stream_queries - 1] = true;

		replaces[Anum_octopus_stream_queries - 1] = true;

		tup = SearchPipelineSysCache1(PIPELINESTREAMRELID, ObjectIdGetDatum(entry->relid));
		if (!HeapTupleIsValid(tup))
			continue;

		newtup = heap_modify_tuple(tup, octopus_stream->rd_att, values, nulls, replaces);

		simple_heap_update(octopus_stream, &newtup->t_self, newtup);
		CatalogUpdateIndexes(octopus_stream, newtup);
 ...
 */
static void
mark_nonexistent_streams(Relation octopus_stream, List *keys)
{
	HeapScanDesc scandesc;
	HeapTuple tup;

	/* delete from the catalog any streams that aren't in the given list */
	scandesc = heap_beginscan_catalog(octopus_stream, 0, NULL);
	while ((tup = heap_getnext(scandesc, ForwardScanDirection)) != NULL)
	{
		ListCell *lc;
		bool found = false;
		Form_octopus_stream row = (Form_octopus_stream) GETSTRUCT(tup);
		...
		if (!found)
		{
			...
			PipelineSysCacheGetAttr(PIPELINESTREAMRELID, tup, Anum_octopus_stream_queries, &isnull);
			...
			newtup = heap_modify_tuple(tup, octopus_stream->rd_att,
...
UpdatePipelineStreamCatalog(void)
{
	...
	octopus_query = heap_open(PipelineQueryRelationOid, AccessShareLock);
	octopus_stream = heap_open(PipelineStreamRelationOid, RowExclusiveLock);
	...
	hash = streams_to_meta(octopus_query);
	keys = update_octopus_stream_catalog(octopus_stream, hash);
	mark_nonexistent_streams(octopus_stream, keys);
	...
	heap_close(octopus_stream, NoLock);
...
GetAllStreamReaders(Oid relid)
{
	...
	raw = PipelineSysCacheGetAttr(PIPELINESTREAMRELID, tup,
						  Anum_octopus_stream_queries, &isnull);
...
{
	Relation octopus_stream = heap_open(PipelineStreamRelationOid, RowExclusiveLock);
	Datum values[Natts_octopus_stream];
	bool nulls[Natts_octopus_stream];
	HeapTuple tup;
	...
	values[Anum_octopus_stream_relid - 1] = ObjectIdGetDatum(relid);
	nulls[Anum_octopus_stream_queries - 1] = true;

	tup = heap_form_tuple(octopus_stream->rd_att, values, nulls);
	simple_heap_insert(octopus_stream, tup);
	CatalogUpdateIndexes(octopus_stream, tup);

	entry_oid = HeapTupleGetOid(tup);

	CommandCounterIncrement();

	/* Record dependency between tuple in octopus_stream and the relation */
...
RemovePipelineStreamById(Oid oid)
{
	...
	simple_heap_delete(octopus_stream, &tuple->t_self);

	ReleaseSysCache(tuple);

	CommandCounterIncrement();

	heap_close(octopus_stream, RowExclusiveLock);
}
...
RelIdIsForOutputStream(Oid id, Oid *cqid)
{
	...
	ScanKeyInit(&skey[0], Anum_octopus_query_osrelid,
			BTEqualStrategyNumber, F_OIDEQ, ObjectIdGetDatum(id));
	scan = heap_beginscan(octopus_query, GetTransactionSnapshot(), 1, skey);
