 ...
 *
 * octopus_query.c
 *	  routines to support manipulation of the octopus_query relation
 *
 * Copyright (c) 2013-2015, Octopus
 *
 * IDENTIFICATION
 *	  src/backend/catalog/octopus_query.c
 *
 *-------------------------------------------------------------------------
 */
#include <math.h>
...
#include "catalog/octopus_query.h"
#include "catalog/octopus_query_fn.h"
#include "catalog/octopus_stream_fn.h"
...
get_next_id(Relation rel)
{
	...
	{
		Form_octopus_query row = (Form_octopus_query) GETSTRUCT(tup);
...
/*
 * Lookup a raw tuple from octopus_query from the given continuous view name
 */
HeapTuple
 ...
 *
 * Adds a CV to the `octopus_query` catalog table.
 */
Oid
DefineContinuousView(Oid relid, Query *query, Oid matrelid, Oid seqrelid, int ttl,
		AttrNumber ttl_attno, Oid *pq_id)
{
	Relation octopus_query;
	HeapTuple tup;
	bool nulls[Natts_octopus_query];
	Datum values[Natts_octopus_query];
	Oid id;
	Oid result;
	char *query_str;
	...
	octopus_query = heap_open(PipelineQueryRelationOid, RowExclusiveLock);

	id = get_next_id(octopus_query);

	Assert(OidIsValid(id));

	MemSet(nulls, 0, sizeof(nulls));

	values[Anum_octopus_query_type - 1] = Int8GetDatum(PIPELINE_QUERY_VIEW);

	values[Anum_octopus_query_id - 1] = Int32GetDatum(id);
	values[Anum_octopus_query_relid - 1] = ObjectIdGetDatum(relid);
	values[Anum_octopus_query_active - 1] = BoolGetDatum(continuous_queries_enabled);
	values[Anum_octopus_query_query - 1] = CStringGetTextDatum(query_str);
	values[Anum_octopus_query_matrelid - 1] = ObjectIdGetDatum(matrelid);
	values[Anum_octopus_query_seqrelid - 1] = ObjectIdGetDatum(seqrelid);
	values[Anum_octopus_query_ttl - 1] = Int32GetDatum(ttl);
	values[Anum_octopus_query_ttl_attno - 1] = Int16GetDatum(ttl_attno);
	values[Anum_octopus_query_step_factor - 1] = Int16GetDatum(QueryGetSWStepFactor(query));

	/* unused */
	values[Anum_octopus_query_tgfn - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_octopus_query_tgnargs - 1] = Int16GetDatum(0);
	values[Anum_octopus_query_tgargs - 1] = DirectFunctionCall1(byteain, CStringGetDatum(""));

	tup = heap_form_tuple(octopus_query->rd_att, values, nulls);

	result = simple_heap_insert(octopus_query, tup);
	CatalogUpdateIndexes(octopus_query, tup);
	...
	heap_close(octopus_query, NoLock);
...
{
	Relation octopus_query = heap_open(PipelineQueryRelationOid, RowExclusiveLock);
	HeapTuple tup = SearchPipelineSysCache1(PIPELINEQUERYID, ObjectIdGetDatum(cvid));
	bool replace[Natts_octopus_query];
	bool nulls[Natts_octopus_query];
	Datum values[Natts_octopus_query];
	HeapTuple new;
	...
	MemSet(nulls, 0 , sizeof(nulls));
	replace[Anum_octopus_query_pkidxid - 1] = true;
	replace[Anum_octopus_query_lookupidxid - 1] = true;
	values[Anum_octopus_query_pkidxid - 1] = ObjectIdGetDatum(pkindid);
	values[Anum_octopus_query_lookupidxid - 1] = ObjectIdGetDatum(lookupindid);

	new = heap_modify_tuple(tup, RelationGetDescr(octopus_query), values, nulls, replace);

	simple_heap_update(octopus_query, &tup->t_self, new);
	CatalogUpdateIndexes(octopus_query, new);
	CommandCounterIncrement();

	ReleaseSysCache(tup);
	heap_close(octopus_query, RowExclusiveLock);
}
...
UpdateContViewRelIds(Oid cvid, Oid cvrelid, Oid osrelid)
{
	...
	MemSet(replace, 0 , sizeof(replace));
	MemSet(nulls, 0 , sizeof(nulls));
	replace[Anum_octopus_query_relid - 1] = true;
	replace[Anum_octopus_query_osrelid - 1] = true;
	replace[Anum_octopus_query_pkidxid - 1] = true;
	replace[Anum_octopus_query_lookupidxid - 1] = true;
	values[Anum_octopus_query_relid - 1] = ObjectIdGetDatum(cvrelid);
	values[Anum_octopus_query_osrelid - 1] = ObjectIdGetDatum(osrelid);
...
GetMatRelName(RangeVar *cvname)
{
	...
	row = (Form_octopus_query) GETSTRUCT(tuple);
...
GetCVNameFromMatRelName(RangeVar *name)
{
	...
	row = (Form_octopus_query) GETSTRUCT(tup);
...
IsSWContView(RangeVar *name)
{
	...
	if (HeapTupleIsValid(tuple))
	{
		Form_octopus_query row = (Form_octopus_query) GETSTRUCT(tuple);
		sw = is_sw(row);
		ReleaseSysCache(tuple);
	}
...
GetContQueryForId(Oid id)
{
	...
	tmp = PipelineSysCacheGetAttr(PIPELINEQUERYRELID, tup, Anum_octopus_query_query, &isnull);
	...
	if (cq->tgnargs > 0)
	{
		...
		val = DatumGetByteaP(PipelineSysCacheGetAttr(PIPELINEQUERYRELID, tup, Anum_octopus_query_tgargs, &isnull));
		Assert(!isnull);
...
{
	Relation octopus_query = heap_open(PipelineQueryRelationOid, AccessShareLock);
	HeapScanDesc scan_desc = heap_beginscan_catalog(octopus_query, 0, NULL);
	HeapTuple tup;
	...
	heap_close(octopus_query, AccessShareLock);
 ...
 *
 * Remove a row from octopus_query along with its associated transition state
 */
void
RemovePipelineQueryById(Oid oid)
{
	...
	octopus_query = heap_open(PipelineQueryRelationOid, ExclusiveLock);
	...
	simple_heap_delete(octopus_query, &tuple->t_self);
...
DefineContinuousTransform(Oid relid, Query *query, Oid typoid, Oid osrelid, Oid fnoid, List *args)
{
	...
	values[Anum_octopus_query_type - 1] = Int8GetDatum(PIPELINE_QUERY_TRANSFORM);

	values[Anum_octopus_query_id - 1] = Int32GetDatum(id);
	values[Anum_octopus_query_relid - 1] = ObjectIdGetDatum(relid);
	values[Anum_octopus_query_active - 1] = BoolGetDatum(continuous_queries_enabled);
	values[Anum_octopus_query_query - 1] = CStringGetTextDatum(query_str);
	values[Anum_octopus_query_tgfn - 1] = ObjectIdGetDatum(fnoid);
	values[Anum_octopus_query_matrelid - 1] = ObjectIdGetDatum(typoid); /* HACK(usmanm): So matrel index works */
	values[Anum_octopus_query_osrelid - 1] = ObjectIdGetDatum(osrelid);
	values[Anum_octopus_query_pkidxid - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_octopus_query_lookupidxid - 1] = ObjectIdGetDatum(InvalidOid);
	...
	if (list_length(args))
	{
		...
		values[Anum_octopus_query_tgnargs - 1] = Int16GetDatum(nargs);
		values[Anum_octopus_query_tgargs - 1] = DirectFunctionCall1(byteain, CStringGetDatum(argsbytes));
	}
	else
	{
		values[Anum_octopus_query_tgnargs - 1] = Int16GetDatum(0);
	...
	/* unused */
	values[Anum_octopus_query_seqrelid - 1] = ObjectIdGetDatum(InvalidOid);
	values[Anum_octopus_query_ttl - 1] = Int32GetDatum(-1);
	values[Anum_octopus_query_ttl_attno - 1] = Int16GetDatum(InvalidAttrNumber);
	values[Anum_octopus_query_step_factor - 1] = Int16GetDatum(0);
...
ContQuerySetActive(Oid id, bool active)
{
	...
	if (row->active != active)
	{
		...
		MemSet(replace, 0 , sizeof(replace));
		MemSet(nulls, 0 , sizeof(nulls));
		replace[Anum_octopus_query_active - 1] = true;
		values[Anum_octopus_query_active - 1] = BoolGetDatum(active);
