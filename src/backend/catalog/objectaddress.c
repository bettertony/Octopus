		...
		case OCLASS_CONTINUOUS_QUERY:
			{
				...
				tup = SearchPipelineSysCache1(PIPELINEQUERYOID,
									  ObjectIdGetDatum(object->objectId));
				if (!HeapTupleIsValid(tup))
					elog(ERROR, "cache lookup failed for octopus_query %u",
						 object->objectId);
				appendStringInfo(&buffer, _("octopus_query %s"),
						get_rel_name(((Form_octopus_query) GETSTRUCT(tup))->relid));
				ReleaseSysCache(tup);
				break;
			}
		...
		case OCLASS_STREAM:
			{
				...
				tup = SearchPipelineSysCache1(PIPELINESTREAMOID,
									  ObjectIdGetDatum(object->objectId));
				if (!HeapTupleIsValid(tup))
					elog(ERROR, "cache lookup failed for octopus_stream %u",
						 object->objectId);
				appendStringInfo(&buffer, _("octopus_stream %s"),
						get_rel_name(((Form_octopus_stream) GETSTRUCT(tup))->relid));
				ReleaseSysCache(tup);
				break;
			}
