				...
				optlines[2] != NULL)
			{
						 ...
						 optlines[5] != NULL)
				{
						...
						)
					{
						...
						"dbname=octopus port=%d host='%s' connect_timeout=5",
								 portnum, host_str);
					}
				}
			}
...
do_init(void)
{
	...
	if (exec_path == NULL)
		exec_path = find_other_exec_or_die(argv0, "octopus-init", "initdb (PostgreSQL) " PG_VERSION "\n");
...
do_start(void)
{
	...
	if (exec_path == NULL)
		exec_path = find_other_exec_or_die(argv0, debug ? "octopus-debug" : "octopus", PG_BACKEND_VERSIONSTR);
...
pgwin32_CommandLine(bool registration)
{
	...
	{
		ret = find_other_exec(argv0, "octopus", PG_BACKEND_VERSIONSTR,
							  cmdPath);
		if (ret != 0)
		{
			write_stderr(_("%s: could not find octopus program executable\n"), progname);
			exit(1);
		}
	}
...
do_help(void)
{
	printf(_("%s is a utility to initialize, start, stop, or control a Octopus server.\n\n"), progname);
	printf(_("Usage:\n"));
...
adjust_data_dir(void)
{
	...
	snprintf(filename, sizeof(filename), "%s/octopus.conf", pg_config);
	...
	if (exec_path == NULL)
		my_exec_path = find_other_exec_or_die(argv0, "octopus", PG_BACKEND_VERSIONSTR);
	else
