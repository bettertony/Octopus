...
report_clusters_compatible(void)
{
	...
	pg_log(PG_REPORT, "\n"
		   "If octopus-upgrade fails after this point, you must re-octopus-init the\n"
						 ...
						 char *deletion_script_file_name)
{
	...
	if (GET_MAJOR_VERSION(old_cluster.major_version) >= 804)
		pg_log(PG_REPORT,
			   "Optimizer statistics are not transferred by octopus-upgrade so,\n"
...
check_for_reg_data_type_usage(ClusterInfo *cluster)
{
	...
	if (found)
	{
		...
		"octopus-upgrade, so this cluster cannot currently be upgraded.  You can\n"
...
get_bin_version(ClusterInfo *cluster)
{
	...
	snprintf(cmd, sizeof(cmd), "\"%s/octopus-ctl\" --version", cluster->bindir);
