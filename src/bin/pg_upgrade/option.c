...
parseCommandLine(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		{
			puts("octopus-upgrade (Octopus) " PIPELINE_VERSION);
			exit(0);
		}
	}
...
usage(void)
{
	printf(_("octopus-upgrade upgrades a Octopus cluster to a different major version.\n\
\nUsage:\n\
  octopus-upgrade [OPTION]...\n\
\n\
Options:\n\
...
\n\
Before running octopus-upgrade you must:\n\
  create a new database cluster (using the new version of octopus-init)\n\
  shutdown the postmaster servicing the old cluster\n\
  shutdown the postmaster servicing the new cluster\n\
\n\
When you run octopus-upgrade, you must provide the following information:\n\
...
\n\
For example:\n\
  octopus-upgrade -d oldCluster/data -D newCluster/data -b oldCluster/bin -B newCluster/bin\n\
