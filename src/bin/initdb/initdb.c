 ...
 * the default database, octopus, are made just by copying template1.
 *
 * To create template1, we run the octopus (backend) program in bootstrap
...
"# allows any local user to connect as any Octopus user, including\n" \
...
set_null_conf(void)
{
	...
	path = psprintf("%s/octopus.conf", pg_data);
...
setup_config(void)
{
	...
	snprintf(path, sizeof(path), "%s/octopus.conf", pg_data);
	...
	sprintf(path, "%s/octopus.auto.conf", pg_data);
...
load_octopus(void)
{
	...
	fputs(_("loading Octopus ... "), stdout);
	fflush(stdout);
	...
	PG_CMD_PUTS("CREATE TABLE _octopus_exec_lock ();\n");
	PG_CMD_PUTS("CREATE FOREIGN DATA WRAPPER stream_fdw HANDLER stream_fdw_handler;\n");
	PG_CMD_PUTS("CREATE SERVER octopus_streams FOREIGN DATA WRAPPER stream_fdw;\n");

	/* Add system level dependencies for the fdw and server */
	PG_CMD_PUTS("INSERT INTO pg_depend SELECT 0,0,0,tableoid,oid,0,'p' FROM pg_foreign_server WHERE srvname = 'octopus_streams';\n");
	...
	PG_CMD_PUTS("UPDATE pg_foreign_server SET srvowner = 0 WHERE srvname = 'octopus_streams';\n");
...
make_postgres(void)
{
	...
	static const char *postgres_setup[] = {
		...
		"COMMENT ON DATABASE octopus IS 'default administrative connection database';\n",
		NULL
	};

	fputs(_("copying template1 to octopus ... "), stdout);
	fflush(stdout);
...
{
	printf(_("%s initializes a Octopus database cluster.\n\n"), progname);
	printf(_("Usage:\n"));
	...
	printf(_("\nIf the data directory is not specified, the environment variable PGDATA\n"
			 "is used.\n"));
	printf(_("\nReport bugs to <eng@octopus.com>.\n"));
}
...
setup_bin_paths(const char *argv0)
{
	...
	if ((ret = find_other_exec(argv0, "octopus", PG_BACKEND_VERSIONSTR,
							   backend_exec)) < 0)
	{
		...
		if (ret == -1)
			fprintf(stderr,
					_("The program \"octopus\" is needed by %s "
			...
			fprintf(stderr,
					_("The program \"octopus\" was found by \"%s\"\n"
...
setup_data_file_paths(void)
{
	...
	set_input(&conf_file, "octopus.conf.sample");
...
main(int argc, char *argv[])
{
	...
	printf(_("\nSuccess. You can now start the database server using:\n\n"
			 "    %s%s%soctopus-ctl%s -D %s%s%s -l logfile start\n\n"),
