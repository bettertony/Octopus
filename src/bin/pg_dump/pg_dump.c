...
is_matrel(Archive *fout, TableInfo *ti, Oid matrel_oid)
{
	 ...
	 */
	appendPQExpBuffer(pq, "SELECT c.oid, pq.matrelid AS matrel FROM octopus_query pq "
...
is_matrel_user_index(Archive *fout, Oid index_oid)
{
	...
	/* TODO(derekjn) Just look it up in octopus_query once we store index OIDs there */
	...
	"JOIN octopus_query pq ON pq.matrelid = i.indrelid "
...
is_seqrel(Archive *fout, Oid oid)
{
	...
	appendPQExpBuffer(pq, "SELECT c.oid, pq.seqrelid AS seqrel FROM octopus_query pq "
...
is_output_stream(Archive *fout, TableInfo *ti)
{
	...
	appendPQExpBuffer(ps, "SELECT osrelid FROM octopus_query WHERE osrelid = %d", ti->dobj.catId.oid);
		...
		char *cv_namespace, char *cv_name)
{
	...
	appendPQExpBuffer(q,
		...
		"FROM pg_class matrel JOIN octopus_query pq ON matrel.oid = pq.matrelid "
...
dumpContinuousView(Archive *fout, TableInfo *ti, PQExpBuffer delq)
{
	...
	appendPQExpBuffer(pq, "SELECT query FROM octopus_views() WHERE schema = '%s' AND name = '%s'",
	...
	resetPQExpBuffer(pq);
	appendPQExpBuffer(pq, "SELECT c.relname AS seqrel FROM octopus_query pq JOIN pg_class c "
...
dumpContinuousTransform(Archive *fout, TableInfo *ti, PQExpBuffer delq)
{
	...
	appendPQExpBuffer(pq, "SELECT tgfn, query FROM octopus_transforms() WHERE schema = '%s' AND name = '%s'",
			ti->dobj.namespace->dobj.name, ti->dobj.name);
	res = ExecuteSqlQueryForSingleRow(fout, pq->data);
	qdef = pg_strdup(PQgetvalue(res, 0, PQfnumber(res, "query")));
	...
	resetPQExpBuffer(pq);
	appendPQExpBuffer(pq, "SELECT unnest(tgargs) FROM octopus_transforms() WHERE schema = '%s' AND name = '%s'",
			ti->dobj.namespace->dobj.name, ti->dobj.name);
	res = ExecuteSqlQuery(fout, pq->data, PGRES_TUPLES_OK);
	resetPQExpBuffer(pq);
...
help(const char *progname)
{
	...
	printf(_("Report bugs to <eng@octopus.com>.\n"));
}
...
dumpForeignServer(Archive *fout, ForeignServerInfo *srvinfo)
{
	...
	/* This is created at initdb time */
	if (strcmp(srvinfo->dobj.name, "octopus_streams") == 0)
		return;
