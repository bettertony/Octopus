...
main(int argc, char *argv[])
{
	...
	if ((ret = find_other_exec(argv[0], "octopus-dump", PGDUMP_VERSIONSTR,
							   pg_dump_bin)) < 0)
	{
	...
	{
		conn = connectDatabase("octopus", connstr, pghost, pgport, pguser,
							   prompt_password, false);
		if (!conn)
		...
		if (!conn)
		{
			fprintf(stderr, _("%s: could not connect to databases \"octopus\" or \"template1\"\n"
...
dumpCreateDB(PGconn *conn)
{
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		else if (strcmp(dbtablespace, "pg_default") != 0 && !no_tablespaces)
		{
			 ...
			 */
			if (strcmp(dbname, "octopus") == 0)
				appendPQExpBuffer(buf, "\\connect template1\n");
			else
				appendPQExpBuffer(buf, "\\connect octopus\n");
