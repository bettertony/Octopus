...
stop_postmaster(void)
{
	if (postmaster_running)
	{
		...
		snprintf(buf, sizeof(buf),
				 "\"%s%soctopus-ctl\" stop -D \"%s/data\" -s -m fast",
				 bindir ? bindir : "",
				 bindir ? "/" : "",
				 temp_instance);
		r = system(buf);
...
drop_database_if_exists(const char *dbname)
{
	...
	psql_command("octopus", "DROP DATABASE IF EXISTS \"%s\"", dbname);
}
...
create_database(const char *dbname)
{
	...
	if (encoding)
		psql_command("octopus", "CREATE DATABASE \"%s\" TEMPLATE=template0 ENCODING='%s'%s", dbname, encoding,
					 (nolocale) ? " LC_COLLATE='C' LC_CTYPE='C'" : "");
	else
		psql_command("octopus", "CREATE DATABASE \"%s\" TEMPLATE=template0%s", dbname,
					 (nolocale) ? " LC_COLLATE='C' LC_CTYPE='C'" : "");
	psql_command(dbname,
...
drop_role_if_exists(const char *rolename)
{
	...
	psql_command("octopus", "DROP ROLE IF EXISTS \"%s\"", rolename);
}
...
create_role(const char *rolename, const _stringlist *granted_dbs)
{
	...
	psql_command("octopus", "CREATE ROLE \"%s\" WITH LOGIN", rolename);
	for (; granted_dbs != NULL; granted_dbs = granted_dbs->next)
	{
		psql_command("octopus", "GRANT ALL ON DATABASE \"%s\" TO \"%s\"",
...
help(void)
{
	printf(_("Octopus regression test driver\n"));
	printf(_("\n"));
	printf(_("Usage:\n  %s [OPTION]... [EXTRA-TEST]...\n"), progname);
	printf(_("\n"));
	printf(_("Options:\n"));
...
regression_main(int argc, char *argv[], init_function ifunc, test_function tfunc)
{
	...
	if (temp_instance)
	{
		...
		snprintf(buf, sizeof(buf),
				 "\"%s%soctopus-init\" -D \"%s/data\" --noclean --nosync%s%s > \"%s/log/initdb.log\" 2>&1",
				 bindir ? bindir : "",
				 bindir ? "/" : "",
				 temp_instance,
				 debug ? " --debug" : "",
		 ...
		 */
		snprintf(buf, sizeof(buf), "%s/data/octopus.conf", temp_instance);
		pg_conf = fopen(buf, "a");
		if (pg_conf == NULL)
		{
