...
-- Verify that octopus_combine entry created
SELECT count(*) FROM octopus_combine WHERE combinefn IN
...
-- octopus_combine entry should be removed
SELECT count(*) FROM octopus_combine WHERE combinefn IN
  (SELECT oid FROM pg_proc WHERE proname='combinable_avg_combine');

-- Dropping the aggregate shouldn't affect any of the functions in octopus_combine
