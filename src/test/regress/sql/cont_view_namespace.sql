...
SELECT schema, name FROM octopus_views() WHERE name LIKE '%test_cvn%' ORDER BY schema, name;
...
SELECT schema, name FROM octopus_streams() WHERE name='test_cvn_stream' ORDER BY schema;
