...
def test_simple_aggs(octopus, clean_db):
    ...
    table_result = list(octopus.execute('SELECT avg(x), sum(y::float8), count(*) FROM test_simple_aggs_t'))
    cv_result = list(octopus.execute('SELECT combine(avg), combine(sum), combine(count) FROM test_simple_aggs'))
...
def test_object_aggs(octopus, clean_db):
    ...
    octopus.create_table('test_object_aggs_t', x='integer', y='float8', s='text')
...
def test_combine_in_view(octopus, clean_db):
    ...
    octopus.execute('CREATE VIEW v AS SELECT combine(avg) FROM test_combine_view')
