...
def _get_pids(grep_str):
  try:
    out = check_output('ps aux | grep "octopus" | grep "%s"' % grep_str,
                       shell=True).split('\n')
...
def get_worker_pids():
  return _get_pids('worker[0-9] \[octopus\]')

def get_combiner_pids():
  return _get_pids('combiner[0-9] \[octopus\]')

def kill_worker():
  return _kill(_get_pid('worker[0-9] \[octopus\]'))


def kill_combiner():
  return _kill(_get_pid('combiner[0-9] \[octopus\]'))
...
def test_postmaster_worker_recovery(octopus, clean_db):
  """
  ...
  while not result and attempts < 10:
    result = octopus.execute("""SELECT pid, query FROM pg_stat_activity WHERE lower(query) LIKE '%%pg_sleep%%'""").first()
    time.sleep(1)
    attempts += 1
  ...
  while attempts < 20:
    try:
      octopus.conn = octopus.engine.connect()
      break
    except:
      time.sleep(1)
      pass
