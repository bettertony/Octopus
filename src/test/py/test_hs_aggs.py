...
def test_rank(octopus, clean_db):
    """
    Verify that continuous rank produces the correct result given random input
    """
    rank, _, _, _, result = _test_hs_agg(octopus, 'rank')

    assert rank == result

def test_dense_rank(octopus, clean_db):
    """
    Verify that continuous dense_rank produces the correct result given random input
    """
    _, dense_rank, _, _, result = _test_hs_agg(octopus, 'dense_rank')
