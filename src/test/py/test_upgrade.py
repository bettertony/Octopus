from base import octopus, clean_db, Octopus
import os
import shutil
import subprocess
import time
...
def test_binary_upgrade(octopus, clean_db):
  ...
  for n in range(32):
    ...
    octopus.create_cv(name, 'SELECT z::text, COUNT(DISTINCT z) AS distinct_count, COUNT(*) FROM stream_%d GROUP BY z' % n)
    rows = [(x, name, name) for x in range(1000)]
  ...
  for n in range(8):
    ...
    octopus.create_stream('namespace.stream_%d' % n, x='int', y='text', z='text')
    octopus.create_cv(name, 'SELECT z::text, COUNT(DISTINCT z) AS distinct_count, COUNT(*) FROM namespace.stream_%d GROUP BY z' % n)
    rows = [(x, name, name) for x in range(1000)]
    octopus.insert('namespace.stream_%d' % n, ('x', 'y', 'z'), rows)
    if n >= 4:
      octopus.execute('CREATE INDEX namespace_idx_%d ON %s(z)' % (n, name))
  ...
  result = subprocess.check_call([
    os.path.join(octopus.bin_dir, 'octopus-upgrade'),
